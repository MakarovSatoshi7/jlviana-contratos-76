...
export function Dashboard() {
  ...
  return <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => <Card key={stat.title} className={`shadow-sm border-border/50 ${stat.bgColor} ${stat.borderColor} transition-all duration-200 hover:shadow-md`}>
        ...
        <Card className="shadow-sm border-border/50">
          ...
          <CardContent>
            <div className="space-y-4">
              {clientes.slice(-5).map((cliente, index) => <div key={cliente.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 transition-colors hover:bg-muted/50">
                  <div>
              ...
              {clientes.length === 0 && <div className="text-center py-8">
        ...
        <Card className="shadow-sm border-border/50">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              return <div key={contrato.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 transition-colors hover:bg-muted/50">
                    <div>
                      <p className="font-medium text-sm">{cliente?.razaoSocial || 'Cliente não encontrado'}</p>
                      <p className="text-xs text-muted-foreground font-mono">
                        R$ {contrato.valorMensalidade.toLocaleString('pt-BR', {
                      minimumFractionDigits: 2
                    })}
                      </p>
                    </div>
              ...
              {contratos.length === 0 && <div className="text-center py-8">
