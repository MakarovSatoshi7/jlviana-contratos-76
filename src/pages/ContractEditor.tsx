      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-muted rounded-lg p-1">
            <Button variant={activeTab === 'form' ? 'default' : 'ghost'} size="sm" onClick={() => setActiveTab('form')} className="flex items-center gap-2">
              <Settings className="h-4 w-4" />
              Dados
            </Button>
            <Button variant={activeTab === 'preview' ? 'default' : 'ghost'} size="sm" onClick={() => setActiveTab('preview')} className="flex items-center gap-2">
              <Eye className="h-4 w-4" />
              Preview
            </Button>
          </div>
          
          {activeTab === 'preview' && <div className="flex items-center gap-2 border rounded-lg px-3 py-1">
          ...
          {canGeneratePDF && <ContratoPDF contrato={formData as Contrato} cliente={selectedCliente} />}
        </div>
      </div>

      <div className="min-h-[calc(100vh-200px)]">
        {activeTab === 'form' && <Card className="w-full max-w-4xl mx-auto">
            <CardHeader>
            ...
            <CardContent>
              <ScrollArea className="h-[calc(100vh-300px)] pr-4">
                <ContratoForm clientes={clientes} onSubmit={handleSaveContract} onChange={handleFormChange} showSaveButton={true} key={Object.keys(formData).length === 0 ? 'reset' : 'editing'} />
              </ScrollArea>
            </CardContent>
          </Card>}

        {activeTab === 'preview' && <Card className="w-full">
            <CardHeader>
